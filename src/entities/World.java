...
public class World {
    ...
    public void draw(Graphics2D g){
        ...
        for (int y = 0; y < 32; y++) {
            for (int x = 0; x < 32; x++) {
                ...
                if(getMap(y, x) == 0){
                    g.drawImage(image, tileX, tileY, 17, 17, null);
                }
                else if(getMap(y, x) == 1){
                    g.drawImage(addedHouseImage, tileX, tileY, 17, 17, null);
                }
    ...
    public void drawHouse(Graphics2D g, House newHouse){
        g.drawImage(newHouse.getImage(), newHouse.getX(), newHouse.getY(), 17, 17, null);
    }

    // Method to draw the cursor
    public void drawCursor(Graphics2D g, Cursor cursor){
        g.drawImage(cursorImage, cursor.getX(), cursor.getY(), 17, 17, null);
    }
